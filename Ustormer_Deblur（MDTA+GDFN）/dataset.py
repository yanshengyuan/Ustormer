...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None, target_transform=None):
        ...
        clean_folder = os.listdir(rgb_dir)
        for x in clean_folder:
            imgs = os.listdir(os.path.join(rgb_dir, x, gt_dir))
            for y in imgs:
                self.clean_filenames.append(os.path.join(rgb_dir, x, gt_dir, y))
        
        self.noisy_filenames = []
        clean_folder = os.listdir(rgb_dir)
        for x in clean_folder:
            imgs = os.listdir(os.path.join(rgb_dir, x, input_dir))
            for y in imgs:
                self.noisy_filenames.append(os.path.join(rgb_dir, x, input_dir, y))
    ...
    def __getitem__(self, index):
        ...
        clean_filename = self.clean_filenames[tar_index][-10:]
        noisy_filename = self.noisy_filenames[tar_index][-10:]
        if clean_filename != noisy_filename:
            print('Warning!! Fatal Error!')
            return None
