...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, img_options=None, target_transform=None):
        ...
        gt_dir = 'ground_truth'
        input_dir = 'rainy_image'
        
        clean_files = os.listdir(os.path.join(rgb_dir, gt_dir))
        self.noisy_files = os.listdir(os.path.join(rgb_dir, input_dir))
        clean_files.sort(key = lambda x: int(x[:-4]))
        
        self.clean_filenames = [os.path.join(rgb_dir, gt_dir, x) for x in clean_files if is_image_file(x)]
        self.noisy_filenames = [os.path.join(rgb_dir, input_dir, x) for x in self.noisy_files if is_image_file(x)]
    ...
    def __getitem__(self, index):
        ...
        name = self.noisy_files[tar_index][:-4]
        flag = 0
        for x in name:
            if x=='_':
                break
                ...
                flag += 1
        pos = int(name[:flag])-1
        clean = torch.from_numpy(np.float32(load_img(self.clean_filenames[pos])))
        
        clean = clean.permute(2,0,1)
        noisy = noisy.permute(2,0,1)

        clean_filename = name[:flag] + '.jpg'
        noisy_filename = self.noisy_files[tar_index]
...
class DataLoaderTrain_Gaussian(Dataset):
    def __init__(self, rgb_dir, noiselevel=5, img_options=None, target_transform=None):
        ...
        self.clean_filenames = [os.path.join(rgb_dir, x) for x in clean_files if is_image_file(x)]
        #self.noisy_filenames = [os.path.join(rgb_dir, 'input', x)       for x in noisy_files if is_jpg_file(x)]
